...
(function () {
	...
	if(navigator.userAgent.match(/(iPhone|iPad)/i))
	{
		...
		if(navigator.userAgent.match(/OS 1_/))
			cp.IOSFlavor = cp.IOS1;
		else if(navigator.userAgent.match(/OS 2_/))
			cp.IOSFlavor = cp.IOS2;
		else if(navigator.userAgent.match(/OS 3_/))
			cp.IOSFlavor = cp.IOS3;
		else if(navigator.userAgent.match(/OS 4_/))
			cp.IOSFlavor = cp.IOS4;
		else if(navigator.userAgent.match(/OS 5_/))
			cp.IOSFlavor = cp.IOS5;
	}
	...
	cp.HotspotQuestion.prototype.addHotspotDiv = function(currDiv,x,y,isCorrect)
	{
		...
		newImg.style.position = 'absolute'; 
		currDiv.appendChild(newImg);
		newImg.style.left = (x - (newImg.clientWidth/2) ) + "px";
		newImg.style.top = (y - (newImg.clientHeight/2) ) + "px";
	...
	cp.CheckBoxInput.prototype.addIfNeeded = function()
	{
		...
		var styleLeft 	= 0 < (this.vbounds.minX) ? 0 : (this.vbounds.minX);
		var styleTop	= 0 < (this.vbounds.minY) ? 0 : (this.vbounds.minY);
		var styleRight 	= cp.model.data.project.w > this.vbounds.maxX ? cp.model.data.project.w : this.vbounds.maxX;
		var styleBottom	= cp.model.data.project.h > this.vbounds.maxY ? cp.model.data.project.h : this.vbounds.maxY;
	...
	cp.TextInput = function(el, args)
	{
		...
		this.element.onkeydown = submitTEBValue;
	...
	cp.AutoShape = function(el, canvasId, args)
	{
		...
		if ( actualParent && undefined != this.parentData.pa && this.bounds ) {
			...
			if ( scaleX < 1.0 && scaleY < 1.0 ) {
				// Since autoshapes are drawn on canvas of size of stage.
				var projWidth	= cp.model.data.project.w;
				var projHeight	= cp.model.data.project.h;
				
				var centerX = ( ( this.bounds.maxX + this.bounds.minX ) / 2 );
				var centerY = ( ( this.bounds.maxY + this.bounds.minY ) / 2 );
				
				var dX = ( projWidth / 2 ) - centerX;
				var dY = ( projHeight / 2 ) - centerY;
				
				var afterScaleX = ( projWidth / 2 ) - ( dX * scaleX );
				var afterScaleY = ( projHeight / 2 ) - ( dY * scaleY );
	...
	cp.Shape.prototype.drawIfNeeded = function()
	{
		...
		if(!this.isMouse)
		{
			...
			this.init = true;
			this.element.originalParent = actualParent;
			styleLeft 	= 0 < this.vbounds.minX ? 0 : this.vbounds.minX;
			styleTop	= 0 < this.vbounds.minY ? 0 : this.vbounds.minY;
			var styleRight 	= cp.model.data.project.w > this.vbounds.maxX ? cp.model.data.project.w : this.vbounds.maxX;
			var styleBottom	= cp.model.data.project.h > this.vbounds.maxY ? cp.model.data.project.h : this.vbounds.maxY;
			styleWidth	= styleRight - styleLeft;
			styleHeight	= styleBottom - styleTop;
			canvas = this.canvas = cp.createCanvas(0, 0, styleWidth, styleHeight,this.element);
		...
		if (this.shape)
		{
			try {
				...
				if (this.shape(gc,imagePath,objectToBeHidden,this.visible,this.divName))
					this.isDrawn = true;
			}
			catch(e)
			{
	...
	cp.FrameSet.prototype.updateFrame = function()
	{
		this.updateOpacity();
	...
	cp.Timeline.prototype.updateFrame = function(reasonForUpdate)
	{
		...
		this.updateToFrame(cpInfoCurrentFrame, false);
	...
	cp.Timeline.prototype.pauseAtFrame = function(frame)
	{
		...
		this.forEachChild(function(child)
		{
			...
			if (isVisible && ((child.pa == frame) || (child.rp_pa && frame == child.rp_pa)))
			{
				...
				if (isHandled || elem.iqb || elem.pausedOnce)
					return;
				if ( undefined != elem.enabled && ! elem.enabled )
					return;

				elem.pausedOnce = true;
				if ( cp.kCPOTAutoShape == elem.type && elem.omms )
					elem.pausedOnce = false; // Don't set this for main master slide.
	...
	cp.Timeline.prototype.updateToFrame = function(frame, force)
	{
	...
	cp.AudioObject = function(audioManager, id, src, fromFrame, toFrame)
	{
	...
	cp.MediaView.prototype = 
	{
		update:function()
		{
			...
			if(this.a && !isNaN(this.a.duration))
			{
				var buffered = this.a.buffered;
				if(buffered.length)
				{
					var downloaded = buffered.end(buffered.length - 1);
					this.downloaded.style.width = ((downloaded/this.a.duration)*cp.MediaView.PROGRESS_WIDTH) + 'px';
					
					if(downloaded == this.a.duration)
					{
						this.status.style.backgroundColor = "#ffffff";
						this.status.innerHTML = "<font color='#000000' style='font-size:8px'>Done</font>";
					}
				}

				this.playHead.style.left = (this.a.currentTime/this.a.duration)*cp.MediaView.PROGRESS_WIDTH + 'px';
			}
		},
	}
	...
	cp.AudioManager = function()
	{
		...
		this.errorCallBackFn = function(e)
		{
			if(-1 == this.src.indexOf('cp_non_existing_media'))
			{
				...
				if(this.waitCount > 0)
				{
					--cp.movie.am.numAudiosWaiting;
					if(cp.movie.am.verbose)
						cp.log('cp.movie.am.numAudiosWaiting = ' + cp.movie.am.numAudiosWaiting);
				}
			}
		}
		...
		this.waitingFn = function(e)
		{
			++this.waitCount;
			if(cp.movie.am.verbose)
				cp.log('wait ' + this.waitCount + ' ' + this.cpSrc + ' ' + (this.cpAudio?this.cpAudio.id:''));
			if(1 == this.waitCount)
			{
				++cp.movie.am.numAudiosWaiting;
				if(cp.movie.am.verbose)
					cp.log('cp.movie.am.numAudiosWaiting = ' + cp.movie.am.numAudiosWaiting);
				if(1 == cp.movie.am.numAudiosWaiting)
					cp.movie.pause(cp.ReasonForPause.WAIT_FOR_RESOURCES);
			}
		}
	...
	cp.AudioManager.prototype = 
	{
		...
		load:function(postLoadFunc)
		{
			...
			if(bgAudioData)
			{
				this.bgAudio = new cp.AudioObject(this, 'bga', bgAudioData.src, 1, bgAudioData.to);//from & to don't make sense for bgAudio
			...
			for(var i =0; i < slideAudioNames.length; ++i)
			{
				if('' != slideAudioNames[i])
				{
					...
					var newAudioObj = new cp.AudioObject(this, slideAudioNames[i], slideAudioData.src, slideAudioData.from, slideAudioData.to);
			...
			for(var j=0; j < slideNames.length; ++j)
			{
				...
				for(var k = 0; k < objectAudioNames.length; ++k)
				{
					if('' != objectAudioNames[k])
					{
						...
						var newAudioObj = new cp.AudioObject(this, objectAudioNames[k], objectAudioData.src, objectAudioData.from, objectAudioData.to);
				...
				for(var k = 0; k < eventAudioNames.length; ++k)
				{
					if('' != eventAudioNames[k])
					{
						...
						var newAudioObj = new cp.AudioObject(this, eventAudioNames[k], eventAudioData.src, -1, -1);
						newAudioObj.deviceSpecificInit();
			...
			{
				this.keyTap = new cp.AudioObject(this, 'pkt', 'ar/KeyClick.mp3', -1, -1);
				this.keyTap.deviceSpecificInit();
			}
			
			this.singletonPlayAudio = new cp.AudioObject(this, 'spa', '', -1, -1);
...
function frameset_mc_draw (ctx,imagePath,objectToBeHidden,visible,divName)
{
	if(imagePath == '')
	{
		return true;
...
function fd(a,b,c,d,e){return frameset_mc_draw(a,b,c,d,e);}
